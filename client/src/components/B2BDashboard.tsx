...
export function B2BDashboard({ factory, demoDrop }: Props) {
  ...
  }, [factory?.updatedAt, factory?.sensors.gas_ppm, factory?.sensors.current_amp]);
          ...
          <div className="h-44">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                <YAxis domain={[0, 'auto']} stroke="#64748b" fontSize={10} width={32} />
                <Tooltip
                  contentStyle={{ background: '#0f172a', border: '1px solid #334155', fontSize: 12 }}
                />
                <ReferenceLine y={GAS_THRESHOLD} stroke="#ef4444" strokeDasharray="4 4" />
                <Line type="monotone" dataKey="gas" stroke="#22d3ee" dot={false} strokeWidth={2} isAnimationActive={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          <div className="h-44">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                <Line type="monotone" dataKey="current" stroke="#a78bfa" dot={false} strokeWidth={2} isAnimationActive={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="rounded-xl border border-cyan-500/30 bg-gradient-to-br from-slate-900 to-cyan-950/40 p-6">
            ...
            <div className="mt-5 space-y-4">
              ...
              <div className="rounded-lg border border-emerald-500/40 bg-slate-950/60 p-4">
                ...
                <p className={`mt-1 text-3xl font-bold ${dropping ? 'text-red-400 line-through' : 'text-emerald-300'}`}>
...
function SafetyGauge({
  ...
  return (
    <svg width="140" height="140" viewBox="0 0 140 140">
      ...
      />
      <text x="70" y="74" textAnchor="middle" className="fill-white text-2xl font-bold" fontSize="28" fontFamily="inherit">
        {score}
      </text>
    </svg>
  );
